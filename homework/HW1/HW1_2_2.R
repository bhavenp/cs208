...
p_rmse <- p_rmse + labs(title= paste("Average RMSE dependent\n on", noise_input, "noise"), x=paste(noise_input, "noise"), y = "Average RMSE") + theme(plot.title = element_text(hjust=0.5), text = element_text(size=17));
...
p_acc <- p_acc + labs(title= paste("Average accuracy of reconstruction\n dependent on", noise_input, "noise"), x=paste(noise_input, "noise"), y = "Average Accuracy") + theme(plot.title = element_text(hjust=0.5), text = element_text(size=17));
...
p_rmse_acc <- p_rmse_acc + labs(title= paste("Average accuracy vs. Average RMSE\n for", noise_input, "noise"), x="Average RMSE", y = "Average Accuracy") + theme(plot.title = element_text(hjust=0.5), text = element_text(size=17));

#create grid for plotting
gs <- grid.arrange(p_rmse, p_acc, p_rmse_acc, ncol=2);
